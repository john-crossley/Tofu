...
open class Tofu: Router {
        ...
        func channelRead(ctx: ChannelHandlerContext, data: NIOAny) {
            ...
            case .head(let header):
                ...
                router.handle(request: req, response: res) { (items: Any...) in
                    res.status = .notFound
                    res.send("[tofu] No middleware handled the request!")
                }
    ...
    open func listen(_ port: Int) {
        ...
        do {
            ...
            print("[tofu] Server running on: ", serverChannel.localAddress!)
            try serverChannel.closeFuture.wait()
        } catch {
            fatalError("[tofu] Failed to start server: \(error)")
        }
    }
}
